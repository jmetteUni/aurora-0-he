...
!
      ...
      UBij=BOUNDS(ng)%UBij
!
      CALL wclock_on (ng, model, 2, 83, MyFile)
     ...
     &                      OCEAN(ng) % vbar,                           &
     &                      OCEAN(ng) % zeta)
      CALL wclock_off (ng, model, 2, 129, MyFile)
!
      RETURN
...
!
      ...
      UBij=BOUNDS(ng)%UBij
!
      CALL wclock_on (ng, model, 2, 725, MyFile)
      CALL ini_zeta_tile (ng, tile, model,                              &
     &                    LBi, UBi, LBj, UBj,                           &
     &                    IminS, ImaxS, JminS, JmaxS,                   &
     &                    kstp(ng), krhs(ng), knew(ng),                 &
     &                    OCEAN(ng) % zeta)
      CALL wclock_off (ng, model, 2, 747, MyFile)
!
      RETURN
...
!
      ...
      UBij=BOUNDS(ng)%UBij
!
      CALL wclock_on (ng, model, 2, 1002, MyFile)
      CALL set_zeta_timeavg_tile (ng, tile, model,                      &
     &                            LBi, UBi, LBj, UBj,                   &
     &                            kstp(ng),                             &
     &                            OCEAN(ng) % zeta,                     &
     &                            COUPLING(ng) % Zt_avg1)
      CALL wclock_off (ng, model, 2, 1010, MyFile)
!
      RETURN
